...
if __name__ == '__main__':
    ...
    # GPU が使える & gpuid >= 0 のときだけ CUDA を使う
    ...
    # ★ ここを修正：必ず device（今はCPU）側にマップしてロードする
    pretrained_dict = torch.load(inference_param_path, map_location=device)
    pretrained_dict = {
    ...
    for i, p in enumerate(sample_pieces):
        ...
        for n in range(samp_per_piece):
            for emotion_env in emotion_events:
                ...
                # ここで Roman 版を確保しておく（Emotion_* は落とす）
                generated = word2event(generated, dset.idx2event)
                events_roman = generated[1:]  # 先頭は Emotion_*

                # いつもの処理：Roman→絶対音高にして MIDI 用に整形
                ...
                # ★ 機能和声（Roman）版を追加で保存
                output_event_roman_path = os.path.join(
                    out_path,
                    'lead_sheet_{}_{}_roman.txt'.format(emotion_env.split('_')[-1], n)
                )
                event_to_txt(events_roman, output_event_path=output_event_roman_path)
