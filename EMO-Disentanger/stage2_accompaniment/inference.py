...
def read_generated_events(events_file, event2idx):
    ...
    for bar in range(len(lead_sheet_bars)):
        filtered_idx_seq = []
        for e in lead_sheet_bars[bar]:
            # event2idx に存在しないトークンは捨てる
            if e not in event2idx:
                # 例えば 'Track_Chord', 'Track_Melody' など
                # print(f"[warn] skip unknown event: {e}")
                continue
            filtered_idx_seq.append(event2idx[e])
        lead_sheet_bars[bar] = filtered_idx_seq
...
def extract_midi_events_from_generation(key, events, relative_melody=False):
    ...
    if relative_melody:
        ...
        for evs in events:
            # Track 切り替え時に状態をリセットしておく
            if evs.startswith('Track_'):
                octave = None
                roman = None
            ...
            if 'Note_Octave' in evs:
                ...
                parts = evs.split('_')
                if len(parts) < 3:
                    # そもそも Degree 情報が壊れているので捨てる
                    continue
                roman = parts[2]

                # ★ Octave が未定義なら、このノートは物理的におかしいので捨てる
                if octave is None:
                    # print(f"[warn] skip Note_Degree {roman} because octave is None")
                    continue
                ...
                if pitch < 21 or pitch > 108:
                    # 上のクランプがあるのでほぼ来ないが、保険で捨てる
                    # print(f"[warn] drop pitch out of range: {pitch}")
                    continue
