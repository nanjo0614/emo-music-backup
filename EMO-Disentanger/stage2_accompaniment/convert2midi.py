...
def event_to_midi(key, events, mode, output_midi_path=None, is_full_event=False,
    ...
    if output_midi_path is not None:
        ...
        # === sanity check: イベント時間の矛盾を全部潰す ===

        # 1) ノートの時間整形
        for inst in midi_obj.instruments:
            cleaned_notes = []
            for n in inst.notes:
                # end < start は物理的におかしいので捨てる
                if n.end < n.start:
                    # print(f"[warn] drop invalid note (end<start): start={n.start}, end={n.end}")
                    continue

                # start が負なら 0 にシフト（end も一緒にずらす）
                if n.start < 0:
                    shift = -n.start
                    n.start = 0
                    n.end += shift

                # end が start より前になったら捨てる（シフト後の保険）
                if n.end < n.start:
                    # print(f"[warn] drop invalid note after shift: start={n.start}, end={n.end}")
                    continue
            ...
            # 時刻順にソート
            cleaned_notes.sort(key=lambda x: (x.start, x.end))
            inst.notes = cleaned_notes

        # 2) time 属性を持つイベント群を一括整形
        def _clean_events_with_time(seq):
            if seq is None:
                return seq
            cleaned = []
            for e in seq:
                if e.time < 0:
                    # time が負なら 0 にクランプ
                    # print(f"[warn] clamp negative time event: {e}")
                    e.time = 0
                cleaned.append(e)
            cleaned.sort(key=lambda x: x.time)
            return cleaned

        # tempo, 拍子, マーカー, 歌詞 など全部時間順に整列&負値クランプ
        if hasattr(midi_obj, "tempo_changes"):
            midi_obj.tempo_changes = _clean_events_with_time(midi_obj.tempo_changes)

        if hasattr(midi_obj, "time_signature_changes"):
            midi_obj.time_signature_changes = _clean_events_with_time(midi_obj.time_signature_changes)

        if hasattr(midi_obj, "markers"):
            midi_obj.markers = _clean_events_with_time(midi_obj.markers)

        if hasattr(midi_obj, "lyrics"):
            midi_obj.lyrics = _clean_events_with_time(midi_obj.lyrics)

        # === ここまでで「時間が負」「順序が逆」のイベントはかなり潰せる ===

        try:
            midi_obj.dump(output_midi_path)
        except ValueError as e:
            # それでもまだ mido が文句言う場合、その曲だけスキップして先に進む
            print(f"[warn] failed to save {output_midi_path}: {e}")
            return
        # === sanity check:ここまで===
